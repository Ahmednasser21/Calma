...
fun RegisterScreen(
    ...
    LoadingStateIndicator(isLoading = state.isLoading) {
        ...
        Column(
            ...
            ErrorStateIndicator(
                error = state.errorMessageResId?.let { stringResource(it) } ?: state.registerError,
                onRetry = null
            )
...
fun PhoneNumberFieldLeadingIcon(selectedCountry: Country, onCountryClick: () -> Unit) {
    Row(
        ...
        modifier = Modifier
            ...
            .padding(start = 8.dp)) {
        AsyncImage(
            model = ImageRequest.Builder(LocalContext.current).data(selectedCountry.flagUrl)
                .build(),
        ...
        Text(
            text = selectedCountry.code, style = MaterialTheme.typography.bodyMedium
        )
        Icon(Icons.Default.ArrowDropDown, contentDescription = null)
        VerticalDivider(
            modifier = Modifier
                .width(1.dp)
                .height(24.dp), color = MaterialTheme.colorScheme.primary
        )
    }
}
                    ...
                    Column(
                        ...
                        OutlinedTextField(
                            ...
                            modifier = Modifier
                                ...
                                .background(
                                    color = Color(0x42C8C8C8), shape = RoundedCornerShape(50.dp)
                                ),
                            placeholder = {
                                Text(
                        ...
                        Box(
                            ...
                            LazyColumn(
                                ...
                                items(items = CountryData.countries.drop(1).filter { country ->
...
fun BirthdayField(
    ...
    val datePickerState = rememberDatePickerState(
        initialSelectedDateMillis = initialDateMillis, selectableDates = object : SelectableDates {
...
fun GenderSelection(selectedGender: String, onGenderClick: (String) -> Unit) {
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        GenderRadioButton(
            text = stringResource(R.string.male), onClick = { _ ->
                onGenderClick("male")
            }, selected = selectedGender == "male"
        )
        GenderRadioButton(
            text = stringResource(R.string.female), onClick = { _ ->
